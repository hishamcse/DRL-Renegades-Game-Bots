# updated according to new versions of libraries as of 25-06-2024
...
def _evaluate_agent(env, n_eval_episodes, policy):
  ...
  for episode in range(n_eval_episodes):
    ...
    while done is False:
      ...
      new_state, reward, done, _, info = env.step(action.cpu().numpy())
...
def record_video(env, policy, out_directory, fps=30):
  ...
  while not done:
    ...
    action, _, _, _  = policy.get_action_and_value(state)
    state, reward, done, _, info = env.step(action.cpu().numpy()) # We directly put next_state = state for recording logic
    img = env.render()
    images.append(img)
...
if __name__ == "__main__":
    ...
    s, _ = envs.reset()
    next_obs = torch.Tensor(s).to(device)
